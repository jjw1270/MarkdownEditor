...
- **UI 언어 자동** — OS 언어에 따라 한국어/영어로 표시됩니다 (환경 변수 `MDE_LANG=ko|en`으로 강제 가능).
...
- **다크 / 라이트 테마** — 한 번 고르면 다음 실행에도 기억합니다. Windows **제목표시줄도 함께** 전환됩니다.
...
GitHub 스타일로 제목·표·코드·인용문을 렌더링합니다. 테마는 우측 상단 버튼으로 전환합니다.
...
| PDF로 내보내기 | `📄 PDF` 버튼 (미리보기 내용 그대로, 라이트 테마로 출력) |
| 이미지 붙여넣기 | 편집 중 `Ctrl+V` — 문서 옆 `images/` 폴더에 저장 + 링크 삽입 (저장된 문서에서만) |
| 다크/라이트 전환 | 우측 상단 테마 버튼 |
