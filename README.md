...
- **자동 업데이트** — 새 버전이 나오면 타이틀 옆 `!` 배지가 강조되어 알려줍니다. 배지를 눌러 **"업데이트하기"** 한 번이면 앱 안에서 내려받아(진행률 표시) 재시작하며 적용됩니다. 설정·세션은 그대로 유지됩니다.
...
타이틀바 우측에 테마(`🌙`/`☀`)·언어(`🌐`) 버튼이 있습니다. `🌐` 메뉴에서 **UI 언어(10개 언어)**를 고를 수 있습니다. 버전은 타이틀 텍스트 오른쪽에 표시되며(클릭하면 저장소·버그 신고·기능 제안 메뉴), 그 옆 `!` 배지로 업데이트를 확인합니다. 창은 메모장처럼 **탭·도구가 타이틀바에 통합된 컴팩트 UI**입니다.
...
| 업데이트 확인 / 적용 | 타이틀 옆 `!` 배지 클릭 — 새 버전이 있으면 배지가 강조되고, 팝업의 **업데이트하기**로 바로 적용 |
...
- 유일한 네트워크 접근은 **업데이트 확인**입니다 — 시작 시(및 수동 확인 시) GitHub API에서 최신 릴리즈 버전만 익명으로 조회하며, 문서·개인 정보는 아무것도 전송하지 않습니다.
