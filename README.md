...
- **우클릭 메뉴** — 미리보기(복사·링크 열기·주소 복사·이미지 확대·찾기), 편집기(잘라내기·복사·붙여넣기·모두 선택 + 핵심 서식)를 우클릭으로.
...
타이틀바 우측에 테마(`🌙`/`☀`)·언어(`🌐`) 버튼이 있습니다. `🌐` 메뉴에서 **UI 언어(10개 언어)**를 고르고 버전 정보도 확인할 수 있습니다. 창은 메모장처럼 **탭·도구가 타이틀바에 통합된 컴팩트 UI**입니다.
...
편집 모드에서는 **서식 바**가 나타납니다 — 굵게/기울임/취소선, 제목(H1~H3), 목록(글머리·번호·체크박스), 인용/코드, 링크/표/구분선을 버튼으로 바로 넣을 수 있어 마크다운 문법을 몰라도 문서를 꾸밀 수 있습니다. 우클릭 메뉴에서는 잘라내기·복사·붙여넣기와 핵심 서식을 쓸 수 있습니다.
