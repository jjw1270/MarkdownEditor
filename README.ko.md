...
- **문서 전용 배율** — `Ctrl+휠`은 탭·툴바를 건드리지 않고 미리보기와 편집 글자만 확대합니다. 배율 표시를 누르면 `−`/`+` 조절기가 열리고, 가운데 배율이나 `Ctrl+0`으로 100% 복원하며 다음 실행에도 유지합니다.
...
| 문서 확대 / 축소 | 문서에서 `Ctrl+휠` · `Ctrl++` / `Ctrl+-` · 배율 표시의 `−`/`+` 조절기 · 가운데 배율 또는 `Ctrl+0`으로 100% 복원 |
