...
- **自動更新** — 啟動時靜默檢查 GitHub Releases；有新版本時，標題旁的 `!` 徽章會亮起提示，可在應用程式內帶進度列完成更新，設定與工作階段全數保留。（版本檢查是本程式唯一的網路存取，絕不傳送任何文件或個人資料。）
...
3. （選用）設為 `.md` 預設應用程式：右鍵 `.md` 檔案 → *開啟檔案* → *選擇其他應用程式* → 選擇 `MarkDownEditor.exe` 並勾選 *一律使用*。
